...
namespace Defra.Trade.ReMoS.AssuranceService.API.Core.Interfaces
{
    public interface IEstablishmentsService
    {
        ...
        public Task<PagedList<LogisticsLocationDto>?> GetActiveLogisticsLocationsForTradePartyAsync(Guid tradePartyId, string? NI_GBFlag, string? searchTerm, string? sortColumn, string? sortDirection, int pageNumber = 1, int pageSize = 50);
        public Task<PagedList<LogisticsLocationDto>?> GetAllLogisticsLocationsForTradePartyAsync(Guid tradePartyId, string? NI_GBFlag, string? searchTerm, string? sortColumn, string? sortDirection, int pageNumber = 1, int pageSize = 50);
