...
public class EstablishmentsController : ControllerBase
{
    ...
    [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(string))]
    [ProducesResponseType(StatusCodes.Status404NotFound, Type = typeof(string))]
    public async Task<IActionResult> GetLogisticsLocationsForTradePartyAsync(Guid tradePartyId)
    {
        IEnumerable<LogisticsLocationDto>? result;
        try
        {
            result = await _establishmentsService.GetLogisticsLocationsForTradePartyAsync(tradePartyId);
            if (result == null)
            {
